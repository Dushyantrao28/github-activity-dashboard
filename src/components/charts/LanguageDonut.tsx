  ...
  return (
    <div className="tooltip-dark">
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        <span style={{ fontWeight: 600, color: '#e6edf3' }}>{d.name}</span>
      </div>
      <div style={{ color: '#7d8590', fontSize: 12, marginTop: 4 }}>{d.percentage}% of codebase</div>
    </div>
  );
};
...
export function LanguageDonut({ data }: { data: LanguageStat[] }) {
  return (
    <div className="card" style={{ padding: '20px 24px' }}>
      ...
      <p style={{ fontSize: 13, color: '#7d8590', marginBottom: 20 }}>Distribution across your repositories</p>
            ...
            <div key={lang.name} style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 13, color: '#e6edf3' }}>{lang.name}</span>
                </div>
                <span style={{ fontSize: 12, color: '#7d8590', fontFamily: 'monospace' }}>{lang.percentage}%</span>
              </div>
