        ...
        <div style={{ background: 'var(--bg-overlay)', padding: 32, borderRadius: 16, border: '1px solid var(--border-muted)', boxShadow: '0 4px 24px rgba(0,0,0,0.2)' }}>
          <form onSubmit={e => e.preventDefault()} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div>
              ...
              <input type="text" placeholder="John Doe" style={{ width: '100%', padding: '12px 16px', background: 'var(--bg-canvas)', border: '1px solid var(--border-muted)', borderRadius: 8, color: 'var(--fg-default)', fontSize: 14, outline: 'none' }} />
            </div>
            
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--fg-default)', marginBottom: 8 }}>Email Address</label>
              <input type="email" placeholder="john@example.com" style={{ width: '100%', padding: '12px 16px', background: 'var(--bg-canvas)', border: '1px solid var(--border-muted)', borderRadius: 8, color: 'var(--fg-default)', fontSize: 14, outline: 'none' }} />
            </div>

            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--fg-default)', marginBottom: 8 }}>Message</label>
              <textarea placeholder="How can we help?" rows={4} style={{ width: '100%', padding: '12px 16px', background: 'var(--bg-canvas)', border: '1px solid var(--border-muted)', borderRadius: 8, color: 'var(--fg-default)', fontSize: 14, outline: 'none', resize: 'vertical' }} />
            </div>

            <button type="submit" className="button-hover" style={{ width: '100%', padding: '14px', background: '#c0b3c6', color: '#161b22', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 700, cursor: 'pointer', marginTop: 8, transition: 'all 0.2s' }}>
              Send Message
            </button>
          </form>
        </div>
